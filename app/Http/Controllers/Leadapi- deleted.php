...
class Leadapi extends Controller
{
    ...
    public function app_open (Request $request) {
        ...
        $insert = DB::table('app_open')->insert(['user_id'=>$id, 'date'=>$date, 'time'=>$time, 'date_time'=>$current]);
        if ($insert) {
    ...
    public function page_time_spend (Request $request) {
        ...
        $insert = DB::table('app_page_time')->insert(['user_id'=>$id,'page_name'=>$page_name,'date'=>$date,'total_time'=>$total_time]);
        if ($insert) {
            $output['response']=true;
    ...
    public function lead_generate (Request $request) {
        ...
        //   if ($count==0) {
           ...
           'messages_status'=>$messages_status,'sms'=>$sms]);
           //echo $data;
        //   if ($data['id']>0) {
                $output['response']=true;
                $output['message']='Lead Generate Successfully';
    ...
    public function my_post (Request $request) {
        ...
        // echo $Autn = DB::table('user')->where(['id'=>$user_id])->count();
        // if ($Autn==0) {
        //     $output['response']=false;
        //     $output['message']='Authentication Failed';
        //     $output['data'] = $data;
