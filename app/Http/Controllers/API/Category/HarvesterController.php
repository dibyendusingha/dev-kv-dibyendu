...
{
    //
    public function harvester_add (Request $request) {
        $output=[];
        $data=[];
        $user_id = auth()->user()->id;

        // Checking limit for add data
        ...
        $crop_type = 'Multi-Crop';
        $cutting_with = '1 - 7 Feets';
        $power_source = 'Self';
        ...
        if ($validator->fails()) {
            ...
            if ($left_image = $request->file('left_image')) {
                ...
                $request->file('left_image')->storeAs('public/harvester', $name);
            ...
            if ($right_image = $request->file('right_image')) {
                ...
                $request->file('right_image')->storeAs('public/harvester', $name1);
            ...
            if ($front_image = $request->file('front_image')) {
                ...
                $request->file('front_image')->storeAs('public/harvester', $name2);
            ...
            if ($back_image = $request->file('back_image')) {
                ...
                $request->file('back_image')->storeAs('public/harvester', $name3);
            ...
            $insert = DB::table('harvester')->insertGetId(['category_id'=>$category_id,'user_id'=>$user_id,'set'=>$set,'type'=>$type,'brand_id'=>$brand_id,'model_id'=>$model_id,'year_of_purchase'=>$year_of_purchase,
            'crop_type'=>$crop_type,'cutting_with'=>$cutting_with,'power_source'=>$power_source,'description'=>$description,'spec_id'=>$spec_id,'left_image'=>$name,'right_image'=>$name1,
            'front_image'=>$name2,'back_image'=>$name3,'price'=>$price,'rent_type'=>$rent_type,'is_negotiable'=>$is_negotiable,'country_id'=>$country_id,'state_id'=>$state_id,'district_id'=>$district_id,'city_id'=>$city_id,'pincode'=>$pincode,'latlong'=>$latlong,
            'title'=>$title,'status'=>0,'created_at'=>date('Y-m-d H:i:s')]);
    ...
    public function harvester (Request $request) {
        $output=[];
        $data=[];
        $new=[];
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table('harvester')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->count();
            if ($count>0) {
            $same_pincode = DB::table('harvester')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->get();
            foreach ($same_pincode as $val) {
                ...
                $data['crop_type'] = $val->crop_type;
                $data['cutting_with'] = $val->cutting_with;
                $data['power_source'] = $val->power_source;
                $data['spec_id'] = $val->spec_id;
                $data['description'] = $val->description;
                if ($val->left_image=='' || $val->left_image=='NULL') { 
                    $data['left_image'] = '';
                } else {
                    $data['left_image'] = asset("storage/harvester/$val->left_image"); 
                }
                if ($val->right_image=='' || $val->right_image=='NULL') { 
                    $data['right_image'] = '';
                } else {
                    $data['right_image'] = asset("storage/harvester/$val->right_image"); 
                }
                if ($val->front_image=='' || $val->front_image=='NULL') { 
                    $data['front_image'] = '';
                } else {
                    $data['front_image'] = asset("storage/harvester/$val->front_image"); 
                }
                if ($val->back_image=='' || $val->back_image=='NULL') { 
                    $data['back_image'] = '';
                } else {
                    $data['back_image'] = asset("storage/harvester/$val->back_image"); 
                }
                ...
                $data['wishlist_status'] = DB::table('wishlist')->where(['user_id'=>$user_id,'category_id'=>4,'item_id'=>$val->id])->count();
            ...
            $arr = [];
            $same_district = DB::table('harvester')->orderBy('id','desc')->where(['district_id'=>$district_id,'status'=>1])->where('pincode','!=',$zipcode)->skip($skip)->take($take)->get();
            ...
            $same_district_nearest = DB::table('harvester')->where(['id'=>$key])->get();    
    ...
    public function harvester_other_district (Request $request) {
         $i=0;
        $output=[];
        $data=[];
        $new=[];
        ...
        if ($validator->fails()) {
           ...
           $count_data += DB::table('harvester')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->count();
           if ($count_data<20) {
           $goods_vehicle_data = DB::table('harvester')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->skip($skip)->take($take)->get();
           ...
           if ($i==0) {
               $goods_vehicle_data = DB::table('harvester')->orderBy('id','desc')->where(['status'=>1])->skip(0)->take(30)->get();
            foreach ($goods_vehicle_data as $val) {
                $data['id'] = $val->id;
                $data['category_id'] = $val->category_id;
                $data['user_id'] = $val->user_id;
    ...
    public function harvester_by_id (Request $request) {
        $output=[];
        $data=[];
        $user_id = $request->user_id;
        $user_token = $request->user_token;
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table('harvester')->where(['id'=>$id])->count();
            if ($count>0) {
            
            $val = DB::table('harvester')->where(['id'=>$id])->first();
    ...
    public function harvester_data (Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if ($type=='rent') {
            $new = Harvester::harvester_data_rent($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take);
        } else {
            $new = Harvester::harvester_data($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take);
        }
            $output['response']=true;
    ...
    public function harvester_update (Request $request) {
        ...
        $userAuth = DB::table('harvester')->where(['user_id'=>$user_id,'id'=>$item_id])->count();
        if ($userAuth==0) {
            $output['response']=false;
        ...
        if ($validator->fails()) {
            ...
            $harv_data = DB::table('harvester')->where(['id'=>$item_id])->first();
            $harv_id = $harv_data->id;
            $harv_userid = $harv_data->user_id;
                
            
            if ($left_image = $request->file('left_image')) {
                $left_image = date('Y-m-d-H-i-s').rand(1000,9999).$request->file('left_image')->getClientOriginalName();
                $ext = $request->file('left_image')->getClientOriginalExtension();
                $request->file('left_image')->storeAs('public/harvester', $left_image);
            }else {
                $left_image=$harv_data->left_image;
            }
            
            if ($right_image = $request->file('right_image')) {
                $right_image = date('Y-m-d-H-i-s').rand(1000,9999).$request->file('right_image')->getClientOriginalName();
                $ext = $request->file('right_image')->getClientOriginalExtension();
                $request->file('right_image')->storeAs('public/harvester', $right_image);
            }else {
                $right_image=$harv_data->right_image;
            }
            
            if ($front_image = $request->file('front_image')) {
                $front_image = date('Y-m-d-H-i-s').rand(1000,9999).$request->file('front_image')->getClientOriginalName();
                $ext = $request->file('front_image')->getClientOriginalExtension();
                $request->file('front_image')->storeAs('public/harvester', $front_image);
            }else {
                $front_image=$harv_data->front_image;
            }
            
            if ($back_image = $request->file('back_image')) {
                $back_image = date('Y-m-d-H-i-s').rand(1000,9999).$request->file('back_image')->getClientOriginalName();
                $ext = $request->file('back_image')->getClientOriginalExtension();
                $request->file('back_image')->storeAs('public/harvester', $left_image);
            }else {
                $back_image=$harv_data->back_image;
            }

            $set = $request->set ? $request->set : $harv_data->set;
            $type = $request->type ? $request->type : $harv_data->type;
            $year_of_purchase = $request->year_of_purchase ? $request->year_of_purchase : $harv_data->year_of_purchase;
            $crop_type = $request->crop_type ? $request->crop_type : $harv_data->crop_type;
            $cutting_with = $request->cutting_with ? $request->cutting_with : $harv_data->cutting_with;
            $power_source = $request->power_source ? $request->power_source : $harv_data->power_source;
            $spec_id = $request->spec_id ? $request->spec_id : $harv_data->spec_id;
            $description = $request->description ? $request->description : $harv_data->description;
            $price = $request->price ? $request->price : $harv_data->price;
            $rent_type = $request->rent_type ? $request->rent_type : $harv_data->rent_type;
            $is_negotiable = ($request->is_negotiable == "0") ? 0 : (($request->is_negotiable == "1") ? 1 : $harv_data->is_negotiable);
                
            
            
            $update = DB::table('harvester')->where(['id'=>$item_id])->update(['set'=>$set,'type'=>$type,'year_of_purchase'=>$year_of_purchase,
            'crop_type'=>$crop_type,'cutting_with'=>$cutting_with,'power_source'=>$power_source,'description'=>$description,'spec_id'=>$spec_id,'left_image'=>$left_image,'right_image'=>$right_image,
            'front_image'=>$front_image,'back_image'=>$back_image,'price'=>$price,'rent_type'=>$rent_type,'is_negotiable'=>$is_negotiable,'status'=>0,'updated_at'=>date('Y-m-d H:i:s')]);
            if ($update>0) {
                ...
                $mobile = DB::table('user')->where('id',$user_id)->value('mobile');
                sms::post_pending($mobile,4);

                $output['response']=true;
                $output['last_id']=$harv_id; //inserted harvester id
    ...
    /** Harvester View All */
    public function harvesterDistance1(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        foreach($sql as $val){
            ...
            if ($spec_count>0) {
                $specification_arr = DB::table('specifications')
                    ->join('harvesterView', 'specifications.model_id', '=', 'harvesterView.model_id')
                    ->where('specifications.model_id', $val->model_id)
                    ->where('specifications.status', 1)
                    ->select('specifications.*', 'harvesterView.model_id')
                    ->get();
            ...
            $view_lead         = DB::select("SELECT COUNT(*) as count FROM leads_views WHERE category_id = 4 AND post_id = :post_id", ['post_id' => $val->id]);
            $data['view_lead'] = $view_lead[0]->count;

            $call_lead          = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 4 AND post_id = :post_id AND calls_status = 1 ", ['post_id' => $val->id]);
            $data['call_lead']  = $call_lead[0]->count;

            $msg_lead          = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 4 AND post_id = :post_id AND messages_status = 1 ",  ['post_id' => $val->id]);
    ...
    public function harvesterDistance(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        if (Cache::has('harvester_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take)) {
            $new = Cache::get('harvester_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take);
            return [
                'response' => true,
                'message' => 'Harvestor Data',
                'data' => $new,
                'data_on' => 'Cache',
                'status_code'=>200,
                'error' => '',
        ...
        foreach($sql as $val){
            ...
            $boosted = Subscribed_boost::view_all_boosted_products(4,$val->id);
            if($boosted == 0){
        ...
        Cache::put('harvester_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take, $new, 86400);
        return [
            'response' => true,
            'message' => 'Harvestor Data',
            'data' => $new,
