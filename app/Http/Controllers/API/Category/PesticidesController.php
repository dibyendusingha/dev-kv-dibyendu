...
{
    //
    public function pesticides_add (Request $request) {
        $output=[];
        $data=[];
        ...
        if ($validator->fails()) {
            ...
            if ($image1 = $request->file('image1')) {
                ...
                $request->file('image1')->storeAs('public/pesticides', $name);
            ...
            if ($image2 = $request->file('image2')) {
                ...
                $request->file('image2')->storeAs('public/pesticides', $name1);
            ...
            if ($image3 = $request->file('image3')) {
                ...
                $request->file('image3')->storeAs('public/pesticides', $name2);
                
            }else {
                $name2='';
            }
            
            $insert = DB::table('pesticides')->insertGetId(['category_id'=>$category_id,'user_id'=>$user_id,'title'=>$title,'price'=>$price,'is_negotiable'=>$is_negotiable,
    ...
    public function pesticides (Request $request) {
        $output=[];
        $data=[];
        $new = [];
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table('pesticides')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->count();
            if ($count>0) {
            $same_pincode = DB::table('pesticides')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->get();
            foreach ($same_pincode as $val) {
                ...
                if ($val->image1=='' || $val->image1=='NULL') { 
                    ...
                    $data['image1'] = asset("storage/pesticides/$val->image1"); 
                }
                if ($val->image2=='' || $val->image2=='NULL') { 
                    $data['image2'] = '';
                } else {
                    $data['image2'] = asset("storage/pesticides/$val->image2"); 
                }
                if ($val->image3=='' || $val->image3=='NULL') { 
                    $data['image3'] = '';
                } else {
                    $data['image3'] = asset("storage/pesticides/$val->image3"); 
                }
                ...
                $data['wishlist_status'] = DB::table('wishlist')->where(['user_id'=>$user_id,'category_id'=>8,'item_id'=>$val->id])->count();
            ...
            $arr = [];
            $same_district = DB::table('pesticides')->orderBy('id','desc')->where(['district_id'=>$district_id,'status'=>1])->where('pincode','!=',$zipcode)->skip($skip)->take($take)->get();
            ...
            $same_district_nearest = DB::table('pesticides')->where(['id'=>$key])->get();    
    ...
    public function pesticides_other_district (Request $request) {
        $i=0;
        $output=[];
        $data=[];
        $new=[];
        ...
        if ($validator->fails()) {
           ...
           $count_data += DB::table('pesticides')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->count();
           if ($count_data<20) {
           $pesticides_data = DB::table('pesticides')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->skip($skip)->take($take)->get();
            foreach ($pesticides_data as $val) {
                $i++;
                $data['id'] = $val->id;
                $data['category_id'] = $val->category_id;
                $data['user_id'] = $val->user_id;
           ...
           if ($i=0) {
               $pesticides_data_data = DB::table('pesticides')->orderBy('id','desc')->where(['status'=>1])->skip(0)->take(30)->get();
            foreach ($pesticides_data_data as $val) {
                $i++;
                $data['id'] = $val->id;
                $data['category_id'] = $val->category_id;
                $data['user_id'] = $val->user_id;
    ...
    public function pesticides_by_id (Request $request) {
        $output=[];
        $data=[];
        $new=[];
        $user_id = $request->user_id;
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table('pesticides')->where(['id'=>$id])->count();
            if ($count>0) {
            
            $val = DB::table('pesticides')->where(['id'=>$id])->first();
    ...
    public function pesticides_data (Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $new = pesticides::pesticides_data($user_id,$pincode,$district,$state,$app_section,$skip,$take);
    ...
    public function pesticides_update (Request $request) {
        ...
        $userAuth = DB::table('pesticides')->where(['user_id'=>$user_id,'id'=>$item_id])->count();
        if ($userAuth==0) {
            $output['response']=false;
        ...
        if ($validator->fails()) {
            ...
            $pest_data = DB::table('pesticides')->where(['id'=>$item_id])->first();
            $pest_id = $pest_data->id;
            $pest_userid = $pest_data->user_id;
            ...
            if ($image3 = $request->file('image3')) {
                ...
                $image3=$pest_data->image3;
            }


            $title = $request->title ? $request->title : $pest_data->title;
            $price = $request->price ? $request->price : $pest_data->price;
            $description = $request->description ? $request->description : $pest_data->description;
            $is_negotiable = ($request->is_negotiable == "0") ? 0 : (($request->is_negotiable == "1") ? 1 : $pest_data->is_negotiable);
            
           $update = DB::table('pesticides')->where(['id'=>$item_id])->update(['title'=>$title,'price'=>$price,'is_negotiable'=>$is_negotiable,
            'description'=>$description,'image1'=>$image1,'image2'=>$image2,'image3'=>$image3,'status'=>0,'created_at'=>date('Y-m-d H:i:s')]);
            if ($update>0) {
                if (isset(auth()->user()->id)) {
                    $user_id = auth()->user()->id;
                }
                ...
                $mobile = DB::table('user')->where('id',$user_id)->value('mobile');
                sms::post_pending($mobile,8);

                $output['response']=true;
                $output['last_id']=$pest_id; //inserted Pesticides id
    ...
    /** Pesticides View All */
    public function pesticidesDistance1(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        foreach($sql as $val){
            ...
            $view_lead               = DB::select("SELECT COUNT(*) as count FROM leads_views WHERE category_id = 8 AND post_id = :post_id", ['post_id' => $val->id]);
            $data['view_lead']       = $view_lead[0]->count;

            $call_lead               = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 8 AND post_id = :post_id AND calls_status = 1 ", ['post_id' => $val->id]);
            $data['call_lead']       = $call_lead[0]->count;

            $msg_lead                = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 8 AND post_id = :post_id AND messages_status = 1 ",  ['post_id' => $val->id]);
    ...
    public function pesticidesDistance(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        if (Cache::has('pesticides_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take)) {
            $new = Cache::get('pesticides_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take);
            return [
                'response' => true,
                'message' => 'Pesticides Data',
                'data' => $new,
                'data_on' => 'Cache',
                'status_code'=>200,
                'error' => '',
        ...
        foreach($sql as $val){
            ...
            $boosted = Subscribed_boost::view_all_boosted_products(8,$val->id);
            if($boosted == 0){
        ...
        Cache::put('pesticides_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take, $new, 86400);
        return [
            'response' => true,
            'message' => 'Pesticides Data',
            'data' => $new,
