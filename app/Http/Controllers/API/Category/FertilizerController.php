...
{
    //
    public function fertilizer_add (Request $request) {
        $output=[];
        $data=[];
        ...
        if ($validator->fails()) {
            ...
            if ($image1 = $request->file('image1')) {
                ...
                $request->file('image1')->storeAs('public/fertilizers', $name);
            ...
            if ($image2 = $request->file('image2')) {
                ...
                $request->file('image2')->storeAs('public/fertilizers', $name1);
            ...
            if ($image3 = $request->file('image3')) {
                ...
                $request->file('image3')->storeAs('public/fertilizers', $name2);
            ...
            $insert = DB::table('fertilizers')->insertGetId(['category_id'=>$category_id,'user_id'=>$user_id,'title'=>$title,'price'=>$price,'is_negotiable'=>$is_negotiable,
    ...
    public function fertilizer (Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $count = DB::table('fertilizers')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->count();
            if ($count>0) {
            $same_pincode = DB::table('fertilizers')->orderBy('id','desc')->where(['pincode'=>$zipcode,'status'=>1])->get();
            ...
            $arr = [];
            $same_district = DB::table('fertilizers')->orderBy('id','desc')->where(['district_id'=>$district_id,'status'=>1])->where('pincode','!=',$zipcode)->skip($skip)->take($take)->get();
            ...
            $same_district_nearest = DB::table('fertilizers')->where(['id'=>$key])->get();    
    ...
    public function fertilizer_other_district (Request $request) {
        $i=0;
        $output=[];
        $data=[];
        $new=[];
        ...
        if ($validator->fails()) {
           ...
           $count_data += DB::table('fertilizers')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->count();
           if ($count_data<20) {
           $seeds_data = DB::table('fertilizers')->orderBy('id','desc')->where(['district_id'=>$key,'status'=>1])->skip($skip)->take($take)->get();
           ...
           if ($i==0) {
               $seeds_data = DB::table('fertilizers')->orderBy('id','desc')->where(['status'=>1])->skip(0)->take(30)->get();
            foreach ($seeds_data as $val) {
                $data['id'] = $val->id;
                $data['category_id'] = $val->category_id;
                $data['user_id'] = $val->user_id;
    ...
    public function fertilizer_by_id (Request $request) {
        $output=[];
        $data=[];
        $new=[];
        $user_id = $request->user_id;
    ...
    public function fertilizer_data (Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $new = fertilizers::fertilizers_data($user_id,$pincode,$district,$state,$app_section,$skip,$take);
        
            $output['response']=true;
            $output['message']='Fertilizer Data';
            $output['data'] = $new;
            $output['error'] = "";
    ...
    public function fertilizer_update (Request $request) {
        ...
        $userAuth = DB::table('fertilizers')->where(['user_id'=>$user_id,'id'=>$item_id])->count();
        if ($userAuth==0) {
            $output['response']=false;
        ...
        if ($validator->fails()) {
            ...
            $fer_data = DB::table('fertilizers')->where(['id'=>$item_id])->first();
            $fer_id = $fer_data->id;
            $fer_userid = $fer_data->user_id;
            ...
            if ($image3 = $request->file('image3')) {
                ...
                $image3=$fer_data->image3;
            }

            $title = $request->title ? $request->title : $fer_data->title;
            $price = $request->price ? $request->price : $fer_data->price;
            $description = $request->description ? $request->description : $fer_data->description;
            $is_negotiable = ($request->is_negotiable == "0") ? 0 : (($request->is_negotiable == "1") ? 1 : $fer_data->is_negotiable);
            
          $update = DB::table('fertilizers')->where(['id'=>$item_id])->update(['title'=>$title,'price'=>$price,'is_negotiable'=>$is_negotiable,
            ...
            if ($update>0) {
                ...
                $mobile = DB::table('user')->where('id',$user_id)->value('mobile');
                sms::post_pending($mobile,9);

                $output['response']=true;
                $output['last_id']=$fer_id; //inserted Fertilizer id
    ...
    /** fertilizer View All */
    public function fertilizersDistance1(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        foreach($sql as $val){
            ...
            $view_lead               = DB::select("SELECT COUNT(*) as count FROM leads_views WHERE category_id = 9 AND post_id = :post_id", ['post_id' => $val->id]);
            $data['view_lead']       = $view_lead[0]->count;

            $call_lead               = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 9 AND post_id = :post_id AND calls_status = 1 ", ['post_id' => $val->id]);
            $data['call_lead']       = $call_lead[0]->count;

            $msg_lead                = DB::select("SELECT COUNT(*) as count FROM seller_leads WHERE category_id = 9 AND post_id = :post_id AND messages_status = 1 ",  ['post_id' => $val->id]);
    ...
    public function fertilizersDistance(Request $request){
        $output = [];
        $data   = [];
        $new    = [];
        ...
        if (Cache::has('fertilizer_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take)) {
            $new = Cache::get('fertilizer_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take);
            return [
                'response' => true,
                'message' => 'Fertilizer Data',
                'data' => $new,
                'data_on' => 'Cache',
                'status_code'=>200,
                'error' => '',
        ...
        foreach($sql as $val){
            ...
            $boosted = Subscribed_boost::view_all_boosted_products(9,$val->id);
            if($boosted == 0){
        ...
        Cache::put('fertilizer_viewall-'.$type.'-'.$user_id.'-'.$skip.'-'.$take, $new, 86400);
            return [
                        'response' => true,
                        'message' => 'Fertilizer Data',
                        'data' => $new,
