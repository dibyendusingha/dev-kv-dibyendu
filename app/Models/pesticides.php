...
class pesticides extends Model
{
    ...
    protected function get_data_by_where ($where) {
        $new = [];
        $count = DB::table('pesticides')->orderBy('id','desc')->where($where)->count();
        if ($count>0) {
        $array_pest_model = DB::table('pesticides')->orderBy('id','desc')->where($where)->get();
            foreach ($array_pest_model as $val) {
                ...
                $data['view_lead'] = Leads_view::where(['category_id'=>8,'post_id'=>$val->id])->count();
                $data['call_lead'] = Lead::where(['category_id'=>8,'post_id'=>$val->id,'calls_status'=>1])->count();
                $data['msg_lead'] = Lead::where(['category_id'=>8,'post_id'=>$val->id,'messages_status'=>1])->count();
    ...
    protected function get_data_by_where_my_post ($where) {
        $new = [];
        $count = DB::table('pesticides')->orderBy('id','desc')->where('id',$where)->count();
        if ($count>0) {
        $array_pest_model = DB::table('pesticides')->orderBy('id','desc')->where('id',$where)->get();
            foreach ($array_pest_model as $val) {
                ...
                if ($val->image1=='' || $val->image1=='NULL') { 
                    ...
                    //$data['left_image'] = env('APP_URL')."storage/pesticides/$val->image1"; 
                    $data['left_image'] = asset("storage/pesticides/$val->image1");
                }
                if ($val->image2=='' || $val->image2=='NULL') { 
                    $data['right_image'] = '';
                } else {
                    //$data['right_image'] = env('APP_URL')."storage/pesticides/$val->image2"; 
                    $data['right_image'] = asset("storage/pesticides/$val->image2"); 
                }
                if ($val->image3=='' || $val->image3=='NULL') { 
                    $data['front_image'] = '';
                } else {
                    $data['front_image'] = asset("storage/pesticides/$val->image3"); 
                }
    ...
    protected function get_notification_data_by_where ($where,$user_id_db) {
        $data=[];
        $count = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where($where)->count();
        if ($count>0) {
        $array_pest_model = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where($where)->get();
            foreach ($array_pest_model as $val) {
                ...
                $data['wishlist_status'] = DB::table('wishlist')->where(['user_id'=>$user_id_db,'category_id'=>8,'item_id'=>$val->id])->count();
                $data['pincode'] = $val->pincode;
                $data['latlong'] = $val->latlong;
                $data['is_featured'] = $val->is_featured;
                $data['valid_till'] = $val->valid_till;
    ...
    protected function get_notification_data_by_where1 ($where,$user_id_db,$viewed_user_id) {
     ...
     protected function get_filter_data_by_where ($user_id,$where_state,$where_district,$price_start,$price_end,$skip,$take) {
        ...
        if ($price_start=='' || $price_start==NULL) {$orwhereBetween = 'orwhereBetween';$price_start=0; $price_end=0;} 
            ...
            /*$array_model = DB::table('pesticides')
    ...
    protected function pesticides_single ($id,$user_id) {
    ...
    protected function pesticides_data ($user_id,$pincode,$district,$state,$app_section,$skip,$take) {
        ...
        $q1 = DB::table('pesticidesView')
                    ->select('*'
                    , DB::raw("6371 * acos(cos(radians(" .$latitude1. "))
                    * cos(radians(pesticidesView.latitude))
                    * cos(radians(pesticidesView.longitude) - radians(" .$longitude1. "))
    ...
    protected function pesticides_category_page () {
        $array=[];
        $banner_d = DB::table('settings')->where(['status'=>'pestisides_category_page_banner'])->get();
        ...
        $icons_d = DB::table('settings')->where(['status'=>'pestisides_category_page_icon'])->get();
    ...
    protected function search_pesticide_brand ($keyword) {
            $count = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $pesti = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($pesti as $val) {
    ...
    protected function search_pesticide_model ($keyword) {
            $count = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $pesti = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($pesti as $val) {
    ...
    protected function search_pesticide_state ($state_id,$keyword) {
            $count = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $pesti = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($pesti as $val) {
    ...
    protected function search_pesticide_district ($district_id,$keyword) {
            $count = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $pesti = DB::table('pesticides')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($pesti as $val) {
