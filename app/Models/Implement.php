...
class Implement extends Model
{
    use HasFactory;
     ...
     protected function get_data_by_where ($where) {
        $new = [];
        $count = DB::table('implements')->orderBy('id','desc')->where('id',$where)->count();
        if ($count>0) {
        $array_implement_model = DB::table('implements')->orderBy('id','desc')->where('id',$where)->get();
            foreach ($array_implement_model as $val) {
                ...
                $data['view_lead'] = Leads_view::where(['category_id'=>5,'post_id'=>$val->id])->count();
                $data['call_lead'] = Lead::where(['category_id'=>5,'post_id'=>$val->id,'calls_status'=>1])->count();
                $data['msg_lead'] = Lead::where(['category_id'=>5,'post_id'=>$val->id,'messages_status'=>1])->count();
    ...
    protected function get_notification_data_by_where ($where,$user_id_db) {
        $data=[];
        $count = DB::table('implements')->orderBy('id','desc')->where('status',1)->where($where)->count();
        if ($count>0) {
        $array_imple_model = DB::table('implements')->orderBy('id','desc')->where('status',1)->where($where)->get();
            foreach ($array_imple_model as $val) {
                ...
                $data['wishlist_status'] = DB::table('wishlist')->where(['user_id'=>$user_id_db,'category_id'=>5,'item_id'=>$val->id])->count();
                $data['latlong'] = $val->latlong;
                $data['ad_report'] = $val->ad_report;
                $data['status'] = $val->status;
                $data['created_at'] = date("d-m-Y", strtotime($val->created_at));
    ...
    protected function get_filter_data_by_where ($user_id,$set,$type,$where_type,$where_state,$where_district,$where_brand,$where_model,$purchase,$price_start,$price_end,$skip,$take) {
        ...
        if ($price_start=='' || $price_start==NULL) {$orwhereBetween = 'orwhereBetween';$price_start=0; $price_end=0;} else {$orwhereBetween = 'whereBetween';$price_start=$price_start; $price_end=$price_end;}
            ...
            $array_tractor_model = DB::table('implements')
    ...
    protected function implement_single ($id,$user_id) {
    ...
    protected function implement_data_rent ($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take) {
        ...
        $q1 = DB::table('implementView')
                    ->select('*'
                    , DB::raw("6371 * acos(cos(radians(" .$latitude1. "))
                    * cos(radians(implementView.latitude))
                    * cos(radians(implementView.longitude) - radians(" .$longitude1. "))
    ...
    protected function implement_data ($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take) {
    ...
    protected function implement_category_page () {
        $array=[];
        $banner_d = DB::table('settings')->where(['status'=>'implements_category_page_banner'])->get();
        ...
        $icons_d = DB::table('settings')->where(['status'=>'implements_category_page_icon'])->get();
    ...
    protected function search_implement_brand ($brand_id,$keyword) {
            $count = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $implements = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($implements as $val) {
    ...
    protected function search_implement_model ($brand_id,$model_id,$keyword) {
            $count = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['model_id'=>$model_id])->orwhere(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $implements = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['model_id'=>$model_id])->orwhere(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($implements as $val) {
    ...
    protected function search_implement_state ($state_id,$keyword) {
            $count = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $implements = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($implements as $val) {
    ...
    protected function search_implement_district ($district_id,$keyword) {
            $count = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $implements = DB::table('implements')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($implements as $val) {
    ...
    /** Dibyendu Change 22.09.2023 */
    protected function implement_old($type ,$pincode){
        $pinDataCity  = DB::table('city')->where(['pincode'=>$pincode])->first();
        $latitude1    = $pinDataCity->latitude;
        $longitude1   = $pinDataCity->longitude;

        $imp_old = DB::table('implementView')
                    ->select('*'
                    , DB::raw("6371 * acos(cos(radians(" .$latitude1. "))
                    * cos(radians(implementView.latitude))
                    * cos(radians(implementView.longitude) - radians(" .$longitude1. "))
    ...
    /** Dibyendu Change 22.09.2023 */
    protected function implement_rent($type ,$pincode){
        $pinDataCity  = DB::table('city')->where(['pincode'=>$pincode])->first();
        $latitude1    = $pinDataCity->latitude;
        $longitude1   = $pinDataCity->longitude;

        $imp_rent = DB::table('implementView')
                    ->select('*'
                    , DB::raw("6371 * acos(cos(radians(" .$latitude1. "))
                    * cos(radians(implementView.latitude))
                    * cos(radians(implementView.longitude) - radians(" .$longitude1. "))
