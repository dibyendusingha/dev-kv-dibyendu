...
class Harvester extends Model
{
    ...
    protected function get_data_by_where ($where) {
        $new = [];
        $count = DB::table('harvester')->orderBy('id','desc')->where('id',$where)->count();
        if ($count>0) {
        $array_harvester_model = DB::table('harvester')->orderBy('id','desc')->where('id',$where)->get();
            foreach ($array_harvester_model as $val) {
                ...
                $data['view_lead'] = Leads_view::where(['category_id'=>4,'post_id'=>$val->id])->count();
                $data['call_lead'] = Lead::where(['category_id'=>4,'post_id'=>$val->id,'calls_status'=>1])->count();
                $data['msg_lead'] = Lead::where(['category_id'=>4,'post_id'=>$val->id,'messages_status'=>1])->count();
    ...
    protected function get_notification_data_by_where ($where,$user_id_db) {
        $data=[];
        $count = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where($where)->count();
        if ($count>0) {
        $array_harvester_model = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where($where)->get();
            foreach ($array_harvester_model as $val) {
                ...
                $data['wishlist_status'] = DB::table('wishlist')->where(['user_id'=>$user_id_db,'category_id'=>4,'item_id'=>$val->id])->count();
                $data['latlong'] = $val->latlong;
                $data['ad_report'] = $val->ad_report;
                $data['status'] = $val->status;
                $data['created_at'] = date("d-m-Y", strtotime($val->created_at));
    ...
    protected function get_notification_data_by_where1 ($where,$user_id_db,$viewed_user_id) {
     ...
     protected function get_filter_data_by_where ($user_id,$set,$type,$where_type,$where_state,$where_district,$where_brand,$where_model,$purchase,$price_start,$price_end,$skip,$take) {
        ...
        if ($price_start=='' || $price_start==NULL) {$orwhereBetween = 'orwhereBetween';$price_start=0; $price_end=0;} else {$orwhereBetween = 'whereBetween';$price_start=$price_start; $price_end=$price_end;}
            ...
            $array_tractor_model = DB::table('harvester')
    ...
    protected function harvester_single ($id,$user_id) {
    ...
    protected function harvester_data_rent ($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take) {
        ...
        $new=[];
            ...
            $q1 = DB::table('harvesterView')
                    ->select('*'
                    , DB::raw("6371 * acos(cos(radians(" .$latitude1. "))
                    * cos(radians(harvesterView.latitude))
                    * cos(radians(harvesterView.longitude) - radians(" .$longitude1. "))
    ...
    protected function harvester_data ($user_id,$type,$pincode,$district,$state,$app_section,$skip,$take) {
    ...
    protected function harvester_category_page () {
        $array=[];
        $banner_d = DB::table('settings')->where(['status'=>'harvester_category_page_banner'])->get();
        ...
        $icons_d = DB::table('settings')->where(['status'=>'harvester_category_page_icon'])->get();
    ...
    protected function search_harvester_brand ($brand_id,$keyword) {
            $count = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $harvester = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($harvester as $val) {
    ...
    protected function search_harvester_model ($brand_id,$model_id,$keyword) {
            $count = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['model_id'=>$model_id])->orwhere(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $harvester = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['model_id'=>$model_id])->orwhere(['brand_id'=>$brand_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($harvester as $val) {
    ...
    protected function search_harvester_state ($state_id,$keyword) {
            $count = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $harvester = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['state_id'=>$state_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($harvester as $val) {
    ...
    protected function search_harvester_district ($district_id,$keyword) {
            $count = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->count();
            if ($count>0) {
            $harvester = DB::table('harvester')->orderBy('id','desc')->where('status',1)->where(['district_id'=>$district_id])->orwhere('title','like', '%'.$keyword.'%')->get();
            foreach ($harvester as $val) {
    ...
    /** Dibyendu Change 22.09.2023 */
     ...
     /** Dibyendu Change 22.09.2023 */
    protected function harvester_old($type ,$pincode){
     ...
     /** Dibyendu Change 22.09.2023 */
    protected function harvester_rent($type ,$pincode){
