...
<?php
    ...
    $boost_product_count = DB::table('subscribed_boosts')->where('category_id', 8)->where('product_id', $post_id)->where('status', 1)->count();
    $category = DB::table('pesticides')->where('id',$post_id)->first();
                ...
                <div class="col-xl-6 d-flex gap-3 align-items-center justify-content-end">
                    ...
                    if($category_status == 1){
                     ...
                     if ($boost_product_count == 0) { ?>
                        <a href="/krishi-boost-payment/pesticides/{{$post_id}}" class="px-3 py-2 bg-success border border-success text-white rounded"><i class="fa-solid fa-rocket me-1"></i>Boost Post</a>
                    <?php }}
                    ?>
                    ...
                    <select class="form-select w-auto d-inline-block" onChange="transfer(this.value)">
                        ...
                        <option value="tractor-pesticides-{{$data['id']}}">Transfer To Tractor</option>
                        <option value="gv-pesticides-{{$data['id']}}">Transfer To Goods Vehicle</option>
                        <option value="harvester-pesticides-{{$data['id']}}">Transfer To Harvester</option>
                        <option value="implements-pesticides-{{$data['id']}}">Transfer To Implements</option>
                        <option value="seeds-pesticides-{{$data['id']}}">Transfer To Seeds</option>
                        <option value="fertilizer-pesticides-{{$data['id']}}">Transfer To Fertilizer</option>
                        <option value="tyre-pesticides-{{$data['id']}}">Transfer To Tyre</option>
                    </select>
                    <?php } ?>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-4 col-sm-4 col-xl-4 text-xl-end">
                    ...
                    <p>
                        ...
                        Total Views: <?php echo DB::table('leads_views')->where(['category_id' => 8, 'post_id' => $data['id']])->count(); ?> <br />
                        Total Leads: <?php echo DB::table('seller_leads')->where(['category_id' => 8, 'post_user_id' => $data['user_id'], 'post_id' => $data['id']])->count(); ?>
                    </p>
                </div>
                <!--  col.// -->
            </div>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{url('add-offline-lead',[8,$post_id,$user_id])}}" method="post" enctype="multipart/form-data">
                    @csrf
...
<div class="modal_wrapper w-100">
    <div class="edit_modal_sidebar py-5 px-3 scroll_edit">
        ...
        <form method="POST" action="{{route('pesticides.update')}}" enctype="multipart/form-data">
            @csrf
...
<script>
    function status_change(x) {
        ...
        $.ajax({
            url: "/krishi-pesticides-status-change",
            type: "POST",
            data: {
                "_token": "{{ csrf_token() }}",
                status_change: x,
