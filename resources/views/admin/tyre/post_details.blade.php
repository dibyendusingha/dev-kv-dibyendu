...
<?php
    ...
    $boost_product_count = DB::table('subscribed_boosts')->where('category_id', 7)->where('product_id', $post_id)->where('status', 1)->count();

    $category = DB::table('tyres')->where('id',$post_id)->first();
            ...
            <div class="row">
                ...
                <div class="col-xl-6 d-flex gap-3 align-items-center justify-content-end">
                    ...
                    if ($boost_product_count == 0) { ?>
                        <a href="/krishi-boost-payment/tyre/{{$post_id}}" class="px-3 py-2 bg-success border border-success text-white rounded"><i class="fa-solid fa-rocket me-1"></i>Boost Post</a>
                    <?php }} ?>
                   ...
                   <?php if($category_status == 0){ ?>
                    <select class="form-select w-auto d-inline-block" onChange="transfer(this.value)">
                        ...
                        <option value="tractor-tyre-{{$data['id']}}">Transfer To Tractor</option>
                        <option value="gv-tyre-{{$data['id']}}">Transfer To Goods Vehicle</option>
                        <option value="harvester-tyre-{{$data['id']}}">Transfer To Harvester</option>
                        <option value="implements-tyre-{{$data['id']}}">Transfer To Implements</option>
                        <option value="seeds-tyre-{{$data['id']}}">Transfer To Seeds</option>
                        <option value="pesticides-tyre-{{$data['id']}}">Transfer To Pesticides</option>
                        <option value="fertilizer-tyre-{{$data['id']}}">Transfer To Fertilizer</option>
            ...
            <div class="row">
                ...
                <div class="col-md-3 col-sm-3 col-xl-3 text-xl-end">
                    ...
                    <p>
                        ...
                        Total Views: <?php echo DB::table('leads_views')->where(['category_id' => 7, 'post_id' => $data['id']])->count(); ?> <br />
                        Total Leads: <?php echo DB::table('seller_leads')->where(['category_id' => 7, 'post_user_id' => $data['user_id'], 'post_id' => $data['id']])->count(); ?>
                    </p>
                </div>
                <!--  col.// -->
            </div>
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{url('add-offline-lead',[7,$post_id,$user_id])}}" method="post" enctype="multipart/form-data">
                    @csrf
...
<div class="modal_wrapper w-100">
    <div class="edit_modal_sidebar py-5 px-3 scroll_edit">
        ...
        <form method="POST" action="{{route('tyre.update')}}" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <select class="form-select mb-4" aria-label="Default select example" name="brnad" onChange="brand_change(this.value)">
                    ...
                    <?php
                    $brand_array = DB::table('brand')->where(['category_id' => 7])->get();
