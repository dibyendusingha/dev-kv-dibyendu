...
?>
<?php
    $user_count = DB::table('user')->where('id',$data->user_id)->count();
    if($user_count > 0){
        $user_details = DB::table('user')->where('id',$data->user_id)->first();
...
<section class="content-main">
    ...
    <h4 class="mb-3">Banner Ad Details</h4>
    <div class="card mb-4">
            ...
            <div class="row border">
                <div class="col-lg-6 border-end p-4">
                    <div class="benner-img-box d-flex justify-content-center p-3">
                        <img src="{{asset('storage/sponser/'.$data->banner_img)}}" alt="banner-img" class="img-fluid" height="300">
                    </div>
                    ...
                    <div class="banner-creative-details d-flex justify-content-center align-items-center flex-column">
                        ...
                        <ul class="list-style">
                            <li><span class="text-success p-2 fw-bold">Campaign Name:</span> <span>{{$data->campaign_name}}</span></li>
                            <li><span class="text-success p-2 fw-bold">Start Date:</span> <span>{{$data->start_date}}</span></li>
                            <li><span class="text-success p-2 fw-bold">End Date:</span> <span>{{$data->end_date}}</span></li>
                            <li><span class="text-success p-2 fw-bold">State Covered:</span> <span>{{$data->campaign_state_names}}</span></li>
                ...
                <div class="col-lg-6">
                    <div class="invoice-details d-flex justify-content-center align-items-center flex-column h-100">
                        ...
                        <ul class="list-style">
                            <li><span class="text-success p-2 fw-bold">Transaction ID:</span> <span>{{$transaction_id}}</span></li>
                            <li><span class="text-success p-2 fw-bold">Payment Date:</span> <span>{{$payment_date}}</span></li>
                            <li><span class="text-success p-2 fw-bold">Amount:</span> <span>{{$purchased_price}}</span></li>
                            <li><span class="text-success p-2 fw-bold">Validity:</span> <span>{{$end_date}}</span></li>
                            <li><span class="text-success p-2 fw-bold">Invoice No:</span> <span>{{$invoice_no}}</span></li>
                            <li class="text-center  mt-3"><button class="btn btn-success text-white">Download Invoice</button></li>
                        </ul>
                    </div>
                </div>
                <!--  col.// -->
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h4 class="card-title">User's Lead Details Table</h4>
            <table class="table">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                @endif
                <br>
                   @foreach ($banner_lead_details as $banner)
                    <?php 
                      $user = DB::table('user')->where('id',$banner->user_id)->first();
                    ...
                    <tr>
                        ...
                        <td>{{$user->zipcode }}</td>
                        <td>
                            <form name="lead" method="post" action="{{ url('update-banner-status',$banner->id)}}" >
                                @csrf
                                <select name="lead_status" class="action-select">
                                    <option value="1" <?php if($banner->status ==1) {?>selected<?php }?>>Hot Lead</option>
                                    <option value="0"  <?php if($banner->status ==0) {?>selected<?php }?>>Normal Lead</option>
                                </select>
