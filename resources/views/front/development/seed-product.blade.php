...
<?php 
    ...
    $seed = DB::table('seeds')->where('id',$seed_id)->first();
...
?>
    ...
    <div>
                    <?php if (session()->has('KVMobile')) { 
                        ...
                        $wishcount = DB::table('wishlist')->where(['user_id'=>$user_id,'category_id'=>6,'item_id'=>$data['id']])->count();
                        ?>
                        <i class="fa-solid fa-heart <?php if ($wishcount>0) {echo 'active-fav';} ?>" id="add-fav" onclick="addtofav('seed',{{$data['id']}})"></i>
