...
<?php 
    ...
    $tyre = DB::table('tyres')->where('id',$tyre_id)->first();
    $user_id = $tyre->user_id;
...
?>
            ...
            <div class="row">
                <div class="col-lg-7">
                    <div class="d-flex">
                        <div class="product-img-list py-3 px-2">
                            ...
                            <?php if ($data['image3']!='') { ?>
                                    <div class="item">
                                        ...
                                        <img src="{{asset($data['image3'])}}" alt="" class="small-img">
                                    </div>
                            <?php } ?>
                        ...
                        <?php if ($data['status'] == 4) { ?>
                            <img src="{{asset('storage/photo/sold_tag.png')}}" class="img-fluid" alt="" width="50" style="position: absolute;top: 0;object-fit:contain">
                ...
                <div class="col-lg-5 pt-4">
                    <div class="d-flex border rounded px-4 justify-content-around">
                        ...
                        <div class="">
                            <div class="fs-4">
                                <i class="fa-solid fa-location-dot text-success"></i> &nbsp; <span> {{$data['city_name']}}, {{$data['district_name']}} , {{$data['state_name']}}, {{$data['pincode']}} </span>
                            </div>
                            <div class="fs-4">
                                <i class="fa-solid fa-calendar-days text-success"></i> &nbsp; <span> {{$data['created_at']}}</span>
                            </div>
                        </div>
    ...
    <div>
                    <?php if (session()->has('KVMobile')) { 
                        ...
                        $wishcount = DB::table('wishlist')->where(['user_id'=>$user_id,'category_id'=>7,'item_id'=>$data['id']])->count();
                        ?>
                        <i class="fa-solid fa-heart <?php if ($wishcount>0) {echo 'active-fav';} ?>" id="add-fav" onclick="addtofav('tyre',{{$data['id']}})"></i>
                    ...
                    <div class="seller-datails border rounded p-3 mt-3">
                        ...
                        <div class="no-show px-5">
                            ...
                            <?php if ($profile_update==1) { ?>
                                <p class="text-center px-5 py-2 rounded bg-light text-dark border border-2 border-dark">{{$data['mobile']}}</p>
                             
                             <a href="tel:{{$data['mobile']}}" type="button" class="w-100 px-5 py-2 border-0 rounded bg-success fw-bold text-white text-center text-decoration-none"><i class="fa-sharp fa-solid fa-phone"></i> CALL</a> 
                                
                            <?php } else { ?>
                            <p class="text-center px-5 py-2 rounded bg-light text-dark border border-2 border-dark">XXXXXXXXXX</p>
                            <button type="button" class="w-100 px-5 py-2 border-0 rounded  bg-success fw-bold text-white myBtn">
            ...
            <div class="item">
                <div class="rp-list">
                    ...
                    <div class="rp-img-box">
                        ...
                        <?php if(!empty($val_rel['title'])){ ?>
                            <p class="fw-bolder">{{$val_rel['title']}}</p>
                        <?php }else{
                            $brand_arr_data = DB::table('brand')->where(['id'=>$val_rel['brand_id']])->first();
